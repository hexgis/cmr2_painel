  ...
  <client-only>
    ...
    >
      ...
      <l-tile-layer
        ...
        :options="{ ...optionsMap, noWrap: true }"
      />
    ...
    setMinZoomToFitContainer() {
      ...
      this.$nextTick(() => {
        try {
          ...
          const worldWidth = 256; // largura do mundo em pixels no zoom 0
          ...
          console.warn('Erro ao calcular zoom mínimo:', error);
    ...
    cloneLayersFromMainMap() {
      ...
      this.mainMap.eachLayer((layer) => {
        ...
        try {
          ...
          if (clonedLayer) {
            if (clonedLayer.options) {
              clonedLayer.options.noWrap = true;
            }
