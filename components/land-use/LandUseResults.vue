    ...
    <v-row no-gutters>
      ...
      >
        <v-btn
          ...
          color="accent"
          icon
          @click="downloadLandUseGeojson(true)"
        >
          <v-tooltip bottom>
...
<i18n>
{
  "en": {
    ...
    "legend": "Legend:",
    "table-name": "Table Daily LandUse"
  },
  "pt-br": {
...
export default {
  ...
  computed: {
    ...
    isLoadingSearchLandUse() {
      return this.$store.state['land-use'].loadingSearchLandUse;
    },

    isLoadingDownloadGeojson() {
      return this.$store.state['land-use'].loadingDownloadGeojson;
    },

    isLoadingStatistic() {
      return this.$store.state['land-use'].loadingStatistic;
    },

    isLoadingTable() {
      return this.$store.state['land-use'].loadingTable;
    },

    isLoadingStats() {
      return this.$store.state['land-use'].loadingStats;
    },

    isLoadingHeatmap() {
      return this.$store.state['land-use'].loadingHeatmap;
    },

    getActiveLegendItems() {
      return !this.$store.getters['land-use/getActiveLegendItems'].length;
    },

    statsTableLandUse() {
      return this.$store.state['land-use'].stats.tableLandUse;
    },

    // formattedAnalyticData() {
    //   return this.$store.state['land-use'].analyticsData;
    // },

    getHeatMapLandUse() {
      return this.$store.state['land-use'].heatMapLandUse;
    },

    ...mapGetters('land-use', ['getStats']),
  },

  methods: {
    async downloadLandUseGeojson() {
      this.$store.dispatch('land-use/downloadLandUseGeojson');
    },

    // async showTableDialogAnalytics() {
    //   this.analyticDialog = true;
    //   this.$store.dispatch('land-use/getDataAnalyticsLandUse');
    // },

    async showTableDialog() {
      this.tableDialog = true;
      await this.$store.dispatch('land-use/getDataTableLandUse');
    },

    // async updateAnalyticDialog(type) {
    //   await this.$store.dispatch('land-use/getDataAnalyticsLandUse', `urgent_alerts_by_${type}`);
    // },
    ...
    //       break;
    //   }
    //   this.$store.dispatch('land-use/downloadAnalyticCSV', defaultFileName);
    // },

    formattedTableLandUse() {
      const tableLandUse = this.$store.state['land-use'].stats.tableLandUse || [];
      return tableLandUse.map((item) => {
    ...
    closeTableDialog() {
      ...
      this.$store.commit('land-use/clearTableLandUse');
    },

    // closeAnalyticalDialog() {
    //   this.analyticDialog = false;
    //   this.$store.commit('land-use/clearAnalyticsData');
    // },
    ...
    async toggleStages(key, value) {
      this.$store.commit('land-use/toggleStatsStages', { key, value });
      this.$store.dispatch('land-use/updateWmsLandUse');
      this.$store.dispatch('land-use/generateLandUseStats', true);
    },

    async toggleHeatMapLayer(value) {
      if (value) {
        await this.$store.dispatch('land-use/generateHeatmapLandUse');
      }
      this.$store.commit('land-use/setHeatMapLandUse', value);
    },
